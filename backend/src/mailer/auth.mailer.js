...
export const sendVerificationMail = async (email, token) => {
  ...
  await transporter.sendMail({
    from: `"LOK Bank" <${process.env.GMAIL_USER}>`, // sender email
    to: email,
    ...
    html: `
      <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: auto; border: 1px solid rgba(212, 175, 55, 0.3); border-radius: 12px; overflow: hidden; background-color: #ffffff; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
        ...
        <div style="padding: 40px; color: #002117; line-height: 1.6;">
          <h1 style="color: #002117; font-size: 26px; font-weight: 900; text-align: left; margin-bottom: 5px; letter-spacing: -1px;">Create Account</h1>
          ...
          <p style="font-size: 12px; color: #888; text-align: center; margin-bottom: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px;">Secure Authentication Link</p>
    ...
    attachments: [
      {
        filename: 'logo.png',
        path: path.join(__dirname, '../../assets/bank_logo.png'),
        cid: 'logo', // inline reference
      },
    ],
  });
};
